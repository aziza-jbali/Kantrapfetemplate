...
import React, { useState, useEffect } from "react";
import PropTypes from "prop-types";
import { getVehicules, deletevehiculeById, updateVehiculeWithImg ,updateVehiculeStatus} from "../../service/apiGestionvehicules";
...
export default function CardTable({ color }) {
  ...
  } else if (key !== "image" && key !== "file" && key !== "reservations") {
...
;
  ...
  <div className={"relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded " + (color === "light" ? "bg-white" : "bg-blueGray-400 text-white")}>
  <div className="rounded-t mb-0 px-4 py-3 border-0">
    <h3 className={"font-semibold text-xl " + (color === "light" ? "text-blueGray-700" : "text-white")}>
      La table de véhicules :
    </h3>
    <div className="mt-3">
      <input
          ...
          <tbody>
            {Vehicules.map((v) => (
              <tr key={v._id}>
                ...
                <td className="border p-2 text-center text-xl">{v.nom} / {v.modele}</td>
                <td className="border p-2 text-center text-xl">{v.carburant}</td>
                <td className="border p-2 text-center text-xl">{v.statusVehicule}</td>
    ...
    >
      ...
      >
        ...
        <input
          ...
          style={{ width: "100%", padding: "8px", borderRadius: "6px", border: "1px solid #ccc", color:"grey" }}
        />
        ...
        <textarea
          ...
          onChange={handleChange}
          rows={4}
          style={{ width: "100%", padding: "8px", borderRadius: "6px", border: "1px solid #ccc", resize: "none", color:"grey" }}
        />
        ...
        >
          ...
          {formData.image ? (
            <>
              <img
                src={formData.image.startsWith("http") ? formData.image : `http://localhost:5011${formData.image}`}
                alt="Preview"
                style={{ width: "100%", height: "100%", objectFit: "cover" }}
              />
              <button
                type="button"
                onClick={() => setFormData({ ...formData, image: null, file: null })}
                style={{
                  position: "absolute",
                  top: "5px",
                  right: "5px",
...
function headerClass(color) {
  return "px-6 py-3 text-xl uppercase font-semibold text-center border border-solid " + (color === "light" ? "bg-blueGray-50 text-blueGray-500 border-blueGray-100" : "bg-blueGray-400 text-lightBlue-300 border-lightBlue-700");
}
